  ...
  return (
    <div className="space-y-8 border-t border-white/5 pt-12">
      ...
      <Tabs
        ...
        className="w-full"
      >
        <TabsList className="bg-white/[0.03] border border-white/5 rounded-xl mb-8 p-1 w-full sm:w-fit flex items-center gap-1 h-10 overflow-x-auto flex-nowrap select-none">
          <TabsTrigger
            value="all"
            className="font-black text-[10px] md:text-xs uppercase tracking-wider px-5 h-full rounded-lg transition-all duration-300 text-white/50 hover:text-white/80 data-active:bg-blue-600! data-active:text-white! data-active:shadow-[0_0_20px_rgba(37,99,235,0.4)]! cursor-pointer flex items-center justify-center shrink-0"
          >
            All ({allCredits.length})
          </TabsTrigger>
          <TabsTrigger
            value="movies"
            className="font-black text-[10px] md:text-xs uppercase tracking-wider px-5 h-full rounded-lg transition-all duration-300 text-white/50 hover:text-white/80 data-active:bg-blue-600! data-active:text-white! data-active:shadow-[0_0_20px_rgba(37,99,235,0.4)]! flex items-center gap-2 cursor-pointer shrink-0"
          >
        ...
        <TabsContent value="all" className="outline-none animate-in fade-in duration-500">
          {allCreditsPaginated.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white/5 border border-dashed border-white/10 rounded-2xl">
              <p className="text-white/40 font-medium">No acting credits found.</p>
            </div>
          )}
        </TabsContent>

        <TabsContent value="movies" className="outline-none animate-in fade-in duration-500">
          {movieCreditsPaginated.length > 0 ? (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
              {movieCreditsPaginated.map((item) => (
                <MovieCard key={`${item.id}-movie`} movie={item} mediaType="movie" />
              ))}
            </div>
          ) : (
            <div className="text-center py-16 bg-white/5 border border-dashed border-white/10 rounded-2xl">
              <p className="text-white/40 font-medium">No movies found in credits.</p>
            </div>
          )}
        </TabsContent>

        <TabsContent value="tv" className="outline-none animate-in fade-in duration-500">
          {tvCreditsPaginated.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white/5 border border-dashed border-white/10 rounded-2xl">
              <p className="text-white/40 font-medium">No TV shows found in credits.</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
